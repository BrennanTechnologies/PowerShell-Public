function Connect-MgGraphAPI {
	...
	.DESCRIPTION
	    ...
	    Automatically installs Microsoft.Graph.Authentication module if not present.
	    Validates certificate existence and connectivity before returning.

	.PARAMETER SettingsPath
	    Path to the settings.json file containing app registration details.
	    Default: .\settings.json
	    Required JSON structure: TenantId, ClientId, CertificateThumbprint
	...
	.OUTPUTS
	    ...
	    Returns the Microsoft Graph context object with connection details.
	...
	.EXAMPLE
	    ...
	    Connects with verbose output and captures the context object

	.EXAMPLE
	    Connect-MgGraphAPI -Scopes @("Directory.Read.All")
	    Connects with specific scope (for delegated auth scenarios)
